...
public class BLL_Timviec 
{
	...
	public boolean CheckTaiKhoan(String email, String pass)
	{
		...
		for (TaiKhoan i : GetListTaiKhoan()) 
		{
			if(i.getEmail().equals(email.trim()) && i.getPass().equals(Passcheck))
			{
				return true;
			}
		}
	...
	public boolean CheckEmailFormat(String email) {
		...
		String p1 = part[0], p2 = part[1];
		for(int i=0;i<p1.length();i++) {
			if(Character.isAlphabetic(p1.charAt(i))==true) {
				c1 += 1;
			}else if(Character.isDigit(p1.charAt(i))==true) {
				c1 += 1;
			}else if(p1.charAt(i)=='.') {
				if(i==p1.length()-1) {
					c1 = 0;
		...
		if(p2.endsWith(".com")) {
			for(int i=0;i<p2.length()-4;i++) {
				if(Character.isAlphabetic(p2.charAt(i))){
					c2 += 1;
				}
			}
		}
		if(c1==p1.length()&&c2==p2.length()-4) {
			return true;
		}
	return false;
	}
	...
	{
		if(name == null && tdhv == null && loaitotnghiep == null && ngoaingu == null && trinhdonn == null && tinh == null){
			GetAllListHoSo();
		}
		else{
