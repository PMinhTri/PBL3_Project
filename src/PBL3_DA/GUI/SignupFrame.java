		...
		btnDangky.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try 
				{
					...
					tk.setEmail(txtEmail.getText().toLowerCase());
					tk.setPass(pass.getText());
					tk.setUserName(txtHoten.getText());
					tk.setSDT(Integer.valueOf(txtSdt.getText()));
					tk.setType(0);
					if(BLL_Timviec.Instance().CheckPass(pass.getText()) == false || BLL_Timviec.Instance().CheckEmailFormat(txtEmail.getText().toLowerCase()) == false ||
							pass.getText().trim() == "" || txtHoten.getText().trim() == "" || BLL_Timviec.Instance().Isnumber(txtSdt.getText().trim()) == false)
					{
						throw new Exception();
					}
					if(BLL_Timviec.Instance().CheckTaiKhoanByEmail(tk.getEmail()) == false)
					{
						...
						{
							tk.setPass(BLL_Timviec.Instance().md5(nhaplaipass.getText()));
				...
				{
					JOptionPane.showMessageDialog(SignupFrame, "Error! Emai, mật khẩu hoặc số điện thoại không hợp lệ");
				}
			}
		});
